            ...
            for key, new_value in extracted_fields.items():
                ...
                if new_value is not None:
                    if isinstance(new_value, str) and new_value.strip():
                        ...
                        if not existing_value or (isinstance(existing_value, str) and len(new_value) > len(existing_value)):
...
def ingest_company_with_portfolio(organization_id: str, name: str) -> Dict[str, Any]:
    ...
    if not portfolio_data:
        ...
        try:
            if is_fund:
                ...
                investor_queries.upsert_investor(existing)
                logger.info(f"Successfully upserted Fund {organization_id} with extracted fields: {list(extracted_fields.keys()) if extracted_fields else 'no new fields'}")
            else:
                logger.info(f"Upserting company {organization_id} with data: {list(existing.keys())}")
                company_queries.upsert_company(existing)
                logger.info(f"Successfully upserted company {organization_id} with extracted fields: {list(extracted_fields.keys()) if extracted_fields else 'no new fields'}")
        except Exception as e:
    ...
    try:
        ...
        company_queries.upsert_company(existing)
        logger.info(f"Successfully upserted company {organization_id} with {len(portfolio_data_for_storage)} portfolio items")
    except Exception as e:
